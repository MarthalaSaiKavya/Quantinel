...
class Backtest:
    ...
    def run(self):
        ...
        for i in range(self.lookback_days, len(dates) - self.horizon_days, self.rebalance_every):
            t = dates[i]
            ...
            risk = self.risk.estimate(window, t)                               # Layer 3
            target = self.optimizer.solve(forecast, risk)                      # Layer 4
            execed = self.executor.execute(target, data, t)                    # Layer 5

            fwd = {tk: float(closes[tk].loc[t_fwd] / closes[tk].loc[t] - 1) for tk in data.tickers}
            records.append(StepRecord(t, execed.realized_weights, fwd, forecast))
            baseline.append(StepRecord(t, {tk: 0.5 for tk in data.tickers}, fwd, None))  # 50/50 hold
